...
lm_dayl.apr %>%
  bind_rows(lm_prp.oct, 
            lm_dayl.prcp,
            lm_daylapr.daylmay) %>%
  mutate(mod=factor(mod,        #This step is useful if we want the specific order of levels
  ...
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) #Useful if x-axis labels are overlapping
...
When you compare the uncorrelated models, estimate didn't change but with correlated models estimate changes alot, it becomes negative for **mean_dayl.s_April** and the error around it also increases. 
...
PC1 ~ sum_precip_oct * coefficient +
      mean_daylength_jan * coefficient
